// Updated ItemReturns.tsx with ArrowLeftCircle import
import React, { useState, useEffect, useRef } from 'react';
import { Plus, Search, Clock, CheckCircle, XCircle, User, AlertCircle, ArrowLeftCircle } from 'lucide-react'; // Added ArrowLeftCircle
import { getRequests, getItems, createRequest, getApprovers } from '../api';
import { useToast } from "@/hooks/use-toast";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Textarea } from "@/components/ui/textarea"; // New import for reason
...
const ItemReturns: React.FC = () => {
  ...
  const handleCreateRequest = async (formData: {
    ...
    try {
      ...
      }, formData.selectedApproverId, 'item_return'); // Pass type as 'item_return'
      setIsFormOpen(false);
      toast({
        title: "Success",
        description: "Item return request created",
        variant: "default"
      });
      loadRequests();
    } catch (error) {
    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row md:items-center md:justify-between mb-6">
        <div>
          ...
          <p className="text-gray-600 mt-1">Submit and manage item return requests with approval workflow</p>
        </div>
      ...
      {isFormOpen && (
        <div className="bg-white rounded-2xl shadow-xl border border-gray-200 p-8 mb-6 relative overflow-hidden">
          ...
          <ReturnForm onSave={handleCreateRequest} onCancel={() => setIsFormOpen(false)} items={items} approvers={approvers} />
        </div>
      )}
...
const ReturnForm: React.FC<ReturnFormProps> = ({ onSave, onCancel, items, approvers }) => {
  ...
  // Refs for form inputs (adjusted for return fields)
  ...
  // Handle Enter key navigation (adjusted)
  const handleKeyDown = (e: React.KeyboardEvent, nextRef: React.RefObject<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement> | null) => {
    if (e.key === 'Enter') {
      e.preventDefault();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.createdBy || !formData.projectName || !formData.location || !formData.reason || !formData.selectedApproverId) {
          ...
          >
            ...
            <SelectContent className="rounded-xl shadow-lg border-gray-200 max-h-60">
              {approvers.map((approver) => (
                <SelectItem key={approver.id} value={approver.id.toString()} className="px-4 py-2 hover:bg-green-50">
                  {approver.fullName}
                </SelectItem>
              ))}
            </SelectContent>
